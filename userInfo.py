...
class UserInfo:
    ...
    def showUserInfo(self):
        ...
        name  = '{:<{l}}'.format(name,l= 20-len(name.encode('GBK'))+len(name))
        
        stuid = termcolor.colored(self.stuid, 'magenta')
        stuid =  '{:<{l}}'.format(stuid,l = 20-len(stuid.encode('GBK'))+len(stuid))

        college =termcolor.colored(self.college, 'yellow')
        college = '{:<{l}}'.format(college,l= 40-len(college.encode('GBK'))+len(college))

        major =termcolor.colored(self.major, 'blue')
        major = '{:<{l}}'.format(major,l= 40-len(major.encode('GBK'))+len(major))
